...
async function main() {
  ...
  if (process.platform === 'win32') {
    try {
      ...
      console.warn('[prisma-generate] Pre-cleanup warning:', String(e))
    }
  }
  ...
  try {
    ...
    if (process.env.PRISMA_SCHEMA_ENGINE_BINARY || process.env.PRISMA_QUERY_ENGINE_LIBRARY) {
      console.log('[prisma-generate] Using local Prisma engines:', {
    ...
    console.warn('[prisma-generate] Could not preconfigure local engines:', String(e))
  }
  ...
  if (result.error) {
    console.error('[prisma-generate] Spawn error:', result.error)
  }
